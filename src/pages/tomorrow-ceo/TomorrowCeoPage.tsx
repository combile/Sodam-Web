...
import styles from "./TomorrowCeoPage.module.css";
...
import StartupItemRecommendationModal from "../../components/StartupItemRecommendationModal";
...
const getChangeRateFromData = (marketData: any, score: number): number => {
  ...
  const baseRate = Math.floor(score / 10) - 3; // 창업은 조금 더 보수적으로
...
const getCompetitionDataFromCSV = (district: string, industry: string) => {
  ...
  return {
    total_businesses: Math.floor(regionalScore * 1.5) + 40, // 창업은 경쟁이 조금 덜함
    competition_score: Math.floor((regionalScore + industryScore) / 2.5) + 10,
  };
};

const TomorrowCeoPage: React.FC = () => {
  ...
  const loadBusinessInfo = async () => {
    try {
      ...
      if (diagnosisResult) {
        ...
        const selectedLocation = locationNames[diagnosis.answers[4]] || "";
        ...
        // 기존 로직 (진단 결과가 없는 경우)
        localStorage.removeItem("planningBusinessInfo");
      ...
      console.error("창업 계획 정보 로드 실패:", error);
    }
  };

  const handleBusinessSubmit = async (data: BusinessItemData) => {
    try {
      // 실제로는 API에 창업 계획 정보를 저장해야 함
      localStorage.setItem("planningBusinessInfo", JSON.stringify(data));
      ...
      console.error("창업 계획 정보 저장 실패:", error);
      throw error;
    }
  };
  ...
  const startAnalysis = async (businessData: BusinessItemData) => {
    ...
    try {
      ...
      let baseScore = 65; // 기본 점수 (창업은 조금 더 보수적)
      ...
      if (diagnosisResult) {
        ...
        baseScore = Math.max(55, Math.min(90, baseScore)); // 55-90점 범위 제한
      } else {
        // 진단 결과가 없으면 기존 로직
  ...
  const generateScoreData = (
    ...
    let opinions = [
      "선택하신 위치와 업종의 매칭도가 매우 우수합니다! 창업 성공 가능성이 높은 지역으로 판단됩니다. 신중한 준비와 차별화된 전략으로 성공적인 창업을 기대할 수 있습니다.",
      "창업하기에 적합한 상권 환경입니다. 유동인구와 체류시간이 양호하여 안정적인 매출을 기대할 수 있습니다. 마케팅과 고객 서비스에 집중하시면 좋은 결과를 얻을 수 있을 것입니다.",
      "창업 가능성이 있는 지역입니다. 다만 경쟁이 치열한 편이므로 차별화된 서비스와 마케팅 전략이 필요합니다. 충분한 사전 조사와 준비를 통해 성공적인 창업을 준비하세요.",
      "창업을 고려해볼 만한 지역입니다. 일부 개선이 필요한 부분이 있지만, 적절한 전략과 노력으로 성공할 수 있습니다. 상세한 사업 계획 수립과 시장 조사를 권장합니다.",
    ];
    ...
    if (score < 70) selectedOpinion = opinions[3];
    else if (score < 80) selectedOpinion = opinions[2];
  ...
  const handleEditBusiness = () => {
    console.log("handleEditBusiness called, setting showBusinessModal to true");
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>내일은 사장</h1>
        <p className={styles.subtitle}>
          창업을 꿈꾸는 당신을 위한 맞춤형 상권 분석
        </p>
      </div>
      ...
      <div className={styles.userInfo}>
        ...
        >
          {isDiagnosisCompleted ? "분석 시작하기" : "창업 진단 시작"}
        </button>
      </div>

      {/* 페이지 내용 */}
      <div className={styles.startupMotivation}>
        ...
        <div className={styles.motivationText}>
          당신의 창업 아이템이 선택하신 지역에서 얼마나 성공할 수 있을지 AI가
          분석해드립니다. 데이터 기반의 객관적인 평가로 창업 성공률을
          높여보세요.
        </div>
        <div className={styles.motivationFeatures}>
          <div className={styles.motivationFeature}>
            <div className={styles.featureIcon}>
              <Analytics style={{ fontSize: "20px" }} />
            </div>
            <div className={styles.featureTitle}>상권 분석</div>
            <div className={styles.featureText}>
              유동인구, 체류시간, 경쟁도 등 종합 분석
            </div>
          </div>
          <div className={styles.motivationFeature}>
            <div className={styles.featureIcon}>
            ...
            <div className={styles.featureTitle}>성공률 예측</div>
            <div className={styles.featureText}>
              AI 기반 창업 성공 가능성 점수 제공
            </div>
          </div>
          <div className={styles.motivationFeature}>
            <div className={styles.featureIcon}>
            ...
            <div className={styles.featureTitle}>맞춤 전략</div>
            <div className={styles.featureText}>
              개인화된 창업 전략 및 추천사항
            </div>
          </div>
      ...
      <div className={styles.businessInfo}>
        ...
        {businessInfo ? (
          <div className={styles.businessDetails}>
            ...
            <div className={styles.businessItem}>
              <div className={styles.businessLabel}>창업 희망 위치</div>
              <div className={styles.businessValue}>
          ...
          <div className={styles.noBusinessInfo}>
            <div className={styles.noBusinessTitle}>
              창업 아이템 정보가 없습니다
            </div>
            <div className={styles.noBusinessText}>
              창업 성공률 분석을 위해 창업을 계획하는 아이템 정보를 입력해주세요
            </div>
            <button
              className={styles.registerButton}
              onClick={() => {
                console.log("창업 아이템 분석하기 버튼 클릭됨");
                setShowBusinessModal(true);
              }}
            >
              창업 아이템 분석하기
              ...
              <>
                  ...
                  <>
                    ...
                    {isAnalysisStarted && analysisStep >= 4 && (
                      <ScoreCard
                        ...
                        title="창업 성공률 분석"
                        subtitle="선택하신 아이템과 위치의 창업 성공 가능성을 AI가 분석한 결과입니다"
                    ...
                    {isAnalysisStarted && analysisStep >= 4 && (
                      <div className={styles.reportSection}>
                        ...
                        <p className={styles.reportDescription}>
                          전문적인 창업 성공률 분석 보고서를 PDF로 다운로드하고,
                          창업 전략과 실행 방안을 확인해보세요.
                        </p>
                      </div>
                    )}
                  </>
                ...
                {!isDiagnosisCompleted && !showDiagnosisModal && (
                  <div className={styles.diagnosisPrompt}>
                    ...
                    <h3>창업 진단을 시작해보세요</h3>
                    <p>
                      먼저 창업 진단을 통해 현재 상황을 파악하고, 맞춤형 창업
                      성공률 분석 결과를 받아보세요.
                    </p>
                  </div>
                )}
      ...
      <MedicalDiagnosisModal
        ...
        onAnswerSelect={(step) => {
          console.log("내일도 사장 - 분석 단계 업데이트:", step);
          setAnalysisStep(step);
          setIsAnalysisStarted(true);
          ...
          <div className={styles.reportModalContent}>
            ...
            <div className={styles.reportModalBody}>
              <DetailedMarketReport
                marketData={{
                  name: `${businessInfo.industry} 창업 계획`,
                  region: "대전광역시",
          ...
          <div className={styles.modalContainer}>
            ...
            <div className={styles.modalBody}>
              ...
              <p className={styles.modalMessage}>
                창업 상세 분석 보고서를 보려면 먼저 상권분석을 완료해주세요.
              </p>
...
export default TomorrowCeoPage;
